...
export default function Settings({ lang, userSettingsHandler }) {
  const { user, userData } = useAuthContext();
  const { updateDocument, getDocument } = useFirestore("users");
  ...
  const handleSaveSettings = async () => {
    ...
    const userSettings = {
      language:
        language === null ? userData.userSettings.language : language.value,
      sidebarColor:
        sidebarColor === null
          ? userData.userSettings.sidebarColor
          : sidebarColor.color,
      mainTheme:
        mainTheme === null ? userData.userSettings.mainTheme : mainTheme.value,
    };
   
    updateDocument(user.uid, { userSettings });
    setIsPending(false);

    const currentUser = await getDocument(user.uid);
    userSettingsHandler(currentUser);
    navigate("/");
  };
  ...
  return (
    <div className="settings-container">
      <div className="language">
        ...
        <Select options={options} onChange={(option) => setLanguage(option)} />
      </div>
      ...
      {isPending ? (
        <button className="btn btn-settings">
          {translation[lang].savePengingBtn}
        </button>
      ) : (
        <button className="btn btn-settings" onClick={handleSaveSettings}>
